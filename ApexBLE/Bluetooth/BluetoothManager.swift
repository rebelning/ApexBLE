...
protocol ApexBLEConnectionDelegate: AnyObject {
...
class BluetoothManager: NSObject {
    ...
    weak var connectionDelegate: ApexBLEConnectionDelegate?
    ...
    //"com.OmniBLE.bluetoothManagerQueue"
    // MARK: - Synchronization
    private let managerQueue = DispatchQueue(label: "com.ApexBLE.bluetoothManagerQueue", qos: .unspecified)

    override init() {
        super.init()

        managerQueue.sync {
            self.manager = CBCentralManager(delegate: self, queue: managerQueue, options: [CBCentralManagerOptionRestoreIdentifierKey: "com.ApexBLE"]) // "com.OmniBLE"
        }
    }
