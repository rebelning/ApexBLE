...
public enum ApexBLEPumpManagerError: Error {
    case noPodPaired
...
extension ApexBLEPumpManagerError: LocalizedError {
...
//    static let podAlarmNotificationIdentifier = "OmniBLE:\(LoopNotificationCategory.pumpFault.rawValue)"
    ...
    public required convenience init?(rawState: PumpManager.RawStateValue) {
        guard let state = ApexBLEPumpManagerState(rawValue: rawState) else
        {
            return nil
        }
    ...
    public var state: ApexBLEPumpManagerState {
        return lockedState.value
    }

    private func setState(_ changes: (_ state: inout ApexBLEPumpManagerState) -> Void) -> Void {
        return setStateWithResult(changes)
    }

    @discardableResult
    private func mutateState(_ changes: (_ state: inout ApexBLEPumpManagerState) -> Void) -> ApexBLEPumpManagerState {
        return setStateWithResult({ (state) -> ApexBLEPumpManagerState in
            changes(&state)
            return state
        })
    }
    ...
    private func setStateWithResult<ReturnType>(_ changes: (_ state: inout ApexBLEPumpManagerState) -> ReturnType) -> ReturnType {
        var oldValue: ApexBLEPumpManagerState!
    ...
    private let lockedState: Locked<ApexBLEPumpManagerState>
...
extension OmniBLEPumpManager {
    ...
    private func status(for state: ApexBLEPumpManagerState) -> PumpManagerStatus {
    ...
    private func device(for state: ApexBLEPumpManagerState) -> HKDevice {
    ...
    private func basalDeliveryState(for state: ApexBLEPumpManagerState) -> PumpManagerStatus.BasalDeliveryState {
    ...
    private func bolusState(for state: ApexBLEPumpManagerState) -> PumpManagerStatus.BolusState {
        guard let podState = state.podState else {
            return .noBolus
        }
    ...
    private func podCommState(for state: ApexBLEPumpManagerState) -> PodCommState {
        guard let podState = state.podState else {
            return .noPod
        }
    ...
    public func buildPumpStatusHighlight(for state: ApexBLEPumpManagerState, andDate date: Date = Date()) -> PumpStatusHighlight? {
    ...
    public func isRunningManualTempBasal(for state: ApexBLEPumpManagerState) -> Bool {
    ...
    public func buildPumpLifecycleProgress(for state: ApexBLEPumpManagerState) -> PumpLifecycleProgress? {
        switch podCommState {
        case .active:
            if shouldWarnPodEOL,
