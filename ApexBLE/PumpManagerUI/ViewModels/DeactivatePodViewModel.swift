...
public protocol PodDeactivater {
    func deactivatePod(completion: @escaping (ApexBLEPumpManagerError?) -> ())
    func forgetPod(completion: @escaping () -> ())
}

extension ApexBLEPumpManager: PodDeactivater {}
        ...
        if case .finished = state {
            ...
            podDeactivator.deactivatePod { (error) in
                DispatchQueue.main.async {
                    if let error = error {
                        self.state = .resultError(DeactivationError.ApexpodPumpManagerError(error))
                    } else {
...
enum DeactivationError : LocalizedError {
    case ApexpodPumpManagerError(ApexBLEPumpManagerError)
    
    var recoverySuggestion: String? {
        switch self {
        case .ApexpodPumpManagerError:
    ...
    var errorDescription: String? {
        switch self {
        case .ApexpodPumpManagerError(let error):
