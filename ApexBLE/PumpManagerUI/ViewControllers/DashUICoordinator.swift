...
class DashUICoordinator: UINavigationController, PumpManagerOnboarding, CompletionNotifying, UINavigationControllerDelegate {
    ...
    var pumpManager: ApexBLEPumpManager
    ...
    private var pumpManagerType: ApexBLEPumpManager.Type?
    ...
    private func viewControllerForScreen(_ screen: DashUIScreen) -> UIViewController {
        ...
        case .deactivate:
            ...
            return hostedView
        case .settings:
            let viewModel = ApexBLESettingsViewModel(pumpManager: pumpManager)
            viewModel.didFinish = { [weak self] in
                self?.stepFinished()
            }
            ...
            let view = ApexBLESettingsView(viewModel: viewModel, supportedInsulinTypes: allowedInsulinTypes)
            return hostingController(rootView: view)
        case .pairAndPrime:
    ...
    init(pumpManager: ApexBLEPumpManager? = nil, colorPalette: LoopUIColorPalette, pumpManagerSettings: PumpManagerSetupSettings? = nil, allowDebugFeatures: Bool, allowedInsulinTypes: [InsulinType] = [])
    {
        if pumpManager == nil, let pumpManagerSettings = pumpManagerSettings {
            let basalSchedule = pumpManagerSettings.basalSchedule
            let pumpManagerState = ApexBLEPumpManagerState(podState: nil, timeZone: basalSchedule.timeZone, basalSchedule: BasalSchedule(repeatingScheduleValues: basalSchedule.items), insulinType: nil, maximumTempBasalRate: pumpManagerSettings.maxBasalRateUnitsPerHour)
